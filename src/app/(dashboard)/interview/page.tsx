...
function getOpeningMessage(userName: string, context?: string | null): string {
  switch (context) {
    case 'timeline':
...
export default function InterviewPage() {
  ...
  useEffect(() => {
    ...
    // If we already processed these exact params, skip
    ...
    if (startNew) {
      // Start a new chat immediately — show hardcoded opening message
  ...
  async function startNewChat(chatContext: string | null) {
    const userName = session?.user?.name || 'there';
    const opening = getOpeningMessage(userName, chatContext);
    // Show the opening message immediately (no API call)
    setMessages([{ role: 'assistant', content: opening }]);
    setLoading(false);

    // Create interview in background
    try {
      // Fetch voice language preference
      ...
      const response = await fetch('/api/interview/chat', {
        ...
        body: JSON.stringify({ startNew: true, chatContext }),
      });
      const data = await response.json();
      if (data.interview) {
        setInterview(data.interview);
        // Save the opening message to the interview
        await fetch('/api/interview/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ interviewId: data.interview.id, saveOpening: opening }),
        });
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <footer className="bg-white border-t border-slate-100 pb-safe">
        <div className="max-w-3xl mx-auto px-3 sm:px-4 py-3 sm:py-4">
          ...
          {/* Bottom row: mic, record, lang, new ... undo */}
