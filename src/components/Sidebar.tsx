...
const NAV_ITEMS = [
  ...
  },
  {
    id: 'timeline-edit',
    label: 'Timeline',
    href: '/interview?new=true&context=timeline',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    id: 'life-script',
    label: 'Life Story',
    href: '/interview?new=true&context=lifestory',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Sidebar({ expanded, isMobile, onClose, onToggle, chatTitle }: SidebarProps) {
  ...
  function isActive(item: typeof NAV_ITEMS[0]) {
    if (pathname !== '/interview') return false;
    const currentContext = searchParams.get('context');
    if (item.id === 'new') {
      return !activeChatId && !currentContext;
    }
    if (item.id === 'memories') {
      return currentContext === 'memories';
    }
    if (item.id === 'timeline-edit') {
      return currentContext === 'timeline';
    }
    if (item.id === 'relations') {
      return currentContext === 'relations';
    }
    if (item.id === 'life-script') {
      return currentContext === 'lifestory';
    }
    return false;
  }
